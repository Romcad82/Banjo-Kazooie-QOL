...
#if defined(SHORTER_CHEATS) || defined(ADDITIONAL_CHEATS)
static void modify_sSecretsCheatCodes(void);
#endif
...
void maCastle_init(void)
{
    ...
    else
    {
        ...
        if (levelSpecificFlags_get(LEVEL_FLAG_5_TTC_UNKNOWN)) {
            ...
            musicKeepsPlaying();
            timedFunc_set_3(2.0f, (GenFunction_3)transitionToMap, MAP_7_TTC_TREASURE_TROVE_COVE, WARP_TTC_1_LEAKY, 0);
        }
...
static SecretCheatCode sSecretsCheatCodes[62] = {
#else
...
static SecretCheatCode sShortCheats[] = {
    // Go Right On Through Note Door Two
...
#if defined(SHORTER_CHEATS) && defined(ADDITIONAL_CHEATS)
static SecretCheatCode sShortAdditionalCheats[] = {
    // Theres Too Many Cheats To Name Use Just One To Unlock The Game
...
#if defined(SHORTER_CHEATS) || defined(ADDITIONAL_CHEATS)
static void append_cheatCode(SecretCheatCode cheatCodeList[], int *codeIndex) {
    int index = *codeIndex;
    SecretCheatCode *i_ptr = &cheatCodeList[0];

    while (i_ptr->code != NULL) {
        sSecretsCheatCodes[index] = *i_ptr;
        index++;
        i_ptr++;
    }
    sSecretsCheatCodes[index] = *i_ptr;
...
static void modify_sSecretsCheatCodes(void) {
 ...
 #ifdef OPTIONS_MENU
  #ifdef SHORTER_CHEATS
    bool shorterCheatsEnabled = is_qol_feature_enabled(QOL_ID_CHEAT_CODES_OVERHAUL);
  #endif
  #ifdef ADDITIONAL_CHEATS
    bool additionalCheatsEnabled = is_qol_feature_enabled(QOL_ID_ADDITIONAL_CHEATS);
  #endif

  #ifdef SHORTER_CHEATS
    if (shorterCheatsEnabled) {
        append_cheatCode(sShortCheats, &codeIndex);
    } else {
        codeIndex = 51;
    }
   #ifdef ADDITIONAL_CHEATS
    if (shorterCheatsEnabled && additionalCheatsEnabled) {
        append_cheatCode(sShortAdditionalCheats, &codeIndex);
    } else if (additionalCheatsEnabled) {
        append_cheatCode(sAdditionalCheats, &codeIndex);
    }
   #endif
  #elif defined(ADDITIONAL_CHEATS)
    if (additionalCheatsEnabled) {
        codeIndex = 51;
        append_cheatCode(sAdditionalCheats, &codeIndex);
    }
  #endif
...
#ifdef EASIER_CHEATS
 #ifdef OPTIONS_MENU
        if (is_qol_feature_enabled(QOL_ID_CHEAT_CODES_OVERHAUL)) {
...
static bool __maCastle_isFloorTileValidForSecretCheatCode(LetterFloorTile *floor_tile)
{
    ...
    if ((__maCastle_getNumberOfBannedCheatCodesEntered() == 3) && !is_qol_feature_enabled(QOL_ID_CHEAT_CODES_OVERHAUL))
    {
        return FALSE;
    }
 #endif
 ...
 #ifdef OPTIONS_MENU
                && is_qol_feature_enabled(QOL_ID_CHEAT_CODES_OVERHAUL)
 #endif
                ) {
                func_8030E58C(SFX_2B_BULL_MOO_1, 1.5f);
            } else {
