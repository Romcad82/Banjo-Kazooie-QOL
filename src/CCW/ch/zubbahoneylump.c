...
/* 
 * Fixes a bug where the Jiggy spawned from the Pink Jinjo gets mistakenly assigned to "jiggy_actor"
 * and becomes uncollectible when you leave and reenter the beehive.
 * In the vanilla game, you can respawn the Jiggy by recollecting the Jinjos again,
 * but it's needed with JINJO_SAVING enabled since the Jiggy could become permanently uncollectible.
 */
#if defined(BUG_AND_OVERSIGHT_FIXES) || defined(JINJO_SAVING)
        jiggy_actor = (Actor *)(ccw_zubbahoneylump_assign_zubba_jiggy(this->position));
#else
