...
/*
 * For some reason, editing src/CCW/zubbahoneylump.c can cause a crash when loading into MAP_22_CC_INSIDE_CLANKER.
...
Actor *ccw_zubbahoneylump_assign_zubba_jiggy(f32 actorPosition[3]) {
    Actor *jiggy_actor;

 #ifdef OPTIONS_MENU
    if (!is_qol_feature_enabled(QOL_ID_JINJO_SAVING) && !is_qol_feature_enabled(QOL_ID_BUG_AND_OVERSIGHT_FIXES)) {
        jiggy_actor = actorArray_findActorFromActorId(0x46);
        return jiggy_actor;
    }
 #endif

    jiggy_actor = actorArray_findClosestActorFromActorId(actorPosition, ACTOR_46_JIGGY, -1, NULL);
    if (jiggy_actor != NULL) {
        if (chjiggy_getJiggyId(jiggy_actor) != JIGGY_4C_CCW_ZUBBAS) {
            jiggy_actor = NULL; 
        }
    }
    return jiggy_actor;
