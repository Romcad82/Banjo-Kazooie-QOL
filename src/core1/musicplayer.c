...
void func_8025AC7C(enum comusic_e, s32, s32, f32, s32 *, const char *, s32);
...
void func_8025A2FC(s32 arg0, s32 arg1){
    ...
    for (i = 1; i < NUM_MUSIC_SLOTS - 1; i++){
        s16 val = (i + comusicTracks)->track_id; // Doesn't match with comusicTracks[i]
        if (val >= 0){
...
void func_8025A388(s32 arg0, s32 arg1) {
    ...
    for (i = 1; i < NUM_MUSIC_SLOTS - 1; i++) {
        CoMusic *current = (i + comusicTracks); // Doesn't match with comusicTracks[i]
