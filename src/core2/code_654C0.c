#include <ultra64.h>
#include "core1/core1.h"
#include "functions.h"
#include "variables.h"


struct {
    s32 unk0;
    f32 unk4[3];
    f32 unk10[3];
    f32 unk1C;
    f32 unk20[3];
}D_803808C0;

Vtx *vtxList_getVertices(BKVertexList *this) {
    return this->vertices;
}

void vtxList_getBounds_s32(BKVertexList *this, s32 min[3], s32 max[3]) {
    min[0] = this->minCoord[0];
    min[1] = this->minCoord[1];
    min[2] = this->minCoord[2];

    max[0] = this->maxCoord[0];
    max[1] = this->maxCoord[1];
    max[2] = this->maxCoord[2];
}

void vtxList_getBounds_f32(BKVertexList *this, f32 min[3], f32 max[3]) {
    min[0] = this->minCoord[0];
    min[1] = this->minCoord[1];
    min[2] = this->minCoord[2];

    max[0] = this->maxCoord[0];
    max[1] = this->maxCoord[1];
    max[2] = this->maxCoord[2];
}

void vtxList_getBoundsTransformed(BKVertexList *this, f32 min[3], f32 max[3]) {
    Vtx *start_vtx, *end_vtx, *vtx;
    s32 i;
    f32 coord[3];

    start_vtx = this->vertices;
    end_vtx = start_vtx + this->count;

    min[0] = start_vtx->v.ob[0];
    min[1] = start_vtx->v.ob[1];
    min[2] = start_vtx->v.ob[2];

    mlMtx_apply_vec3f(min, min);

    max[0] = min[0];
    max[1] = min[1];
    max[2] = min[2];

    for (vtx = start_vtx + 1; vtx < end_vtx; vtx++) { 
        coord[0] = vtx->v.ob[0];
        coord[1] = vtx->v.ob[1];
        coord[2] = vtx->v.ob[2];
        mlMtx_apply_vec3f(coord, coord);

        for (i = 0; i < 3; i++) {
            if (coord[i] < min[i]) {
                min[i] = coord[i];
            }
            if (max[i] < coord[i]) {
                max[i] = coord[i];
            }
        }
    }
}

void vtxList_func_802EC680(BKVertexList *this, s32 arg1, f32 arg2[3], f32 arg3[3]) {
    Vtx *start_vtx, *end_vtx, *vtx;
    s32 i;
    f32 coord[3];

    start_vtx = this->vertices;
    end_vtx = start_vtx + this->count;

    for (vtx = start_vtx; vtx < end_vtx; vtx++) {
        coord[0] = vtx->v.ob[0];
        coord[1] = vtx->v.ob[1];
        coord[2] = vtx->v.ob[2];
        mlMtx_apply_vec3f(coord, coord);
        if ((vtx == start_vtx) || (coord[1] < arg2[1])) {
            arg2[1] = coord[1];
        }
    }

    arg3[1] = arg2[1] + arg1;

    for (vtx = start_vtx; vtx < end_vtx; vtx++) {
        coord[0] = (f32) vtx->v.ob[0];
        coord[1] = (f32) vtx->v.ob[1];
        coord[2] = (f32) vtx->v.ob[2];
        mlMtx_apply_vec3f(coord, coord);
        if (coord[1] < arg3[1]) {

            arg2[0] = coord[0];
            arg2[2] = coord[2];
            
            arg3[0] = coord[0];
            arg3[2] = coord[2];
            break;
        } 
    }

    for (vtx = vtx + 1; vtx < end_vtx; vtx++) {
        coord[0] = (f32) vtx->v.ob[0];
        coord[1] = (f32) vtx->v.ob[1];
        coord[2] = (f32) vtx->v.ob[2];
        mlMtx_apply_vec3f(coord, coord);
        if (coord[1] < arg3[1]) {
            for (i = 0; i < 3; i += 2) {
                if (coord[i] < arg2[i]) {
                    arg2[i] = coord[i];
                }
                if (arg3[i] < coord[i]) {
                    arg3[i] = coord[i];
                }
            }
        }
    }
}

void vtxList_getVtxRange(BKVertexList *this, Vtx **begin_ptr, Vtx **end_ptr) {
    *begin_ptr = this->vertices;
    *end_ptr = &(*begin_ptr)[this->count];
}

s32 vtxList_getVtxCount(BKVertexList *this) {
    return this->count;
}

f32 vtxList_getGlobalNorm(BKVertexList *this) {
    return this->global_norm;
}

void vtxList_getCenterAndNorm(BKVertexList *this, f32 center[3], f32 *norm) {
    center[0] = this->centerCoord[0];
    center[1] = this->centerCoord[1];
    center[2] = this->centerCoord[2];
    *norm = this->local_norm; 
}

f32 vtxList_getLocalNorm(BKVertexList *this) {
    return this->local_norm;
}

void vtxList_free(BKVertexList *this) {
    bk_free(this);
}

BKVertexList *vtxList_clone(BKVertexList *this) {
    BKVertexList *new_ptr;
    u32 list_size;
    
    list_size = sizeof(BKVertexList) + this->count * sizeof(Vtx);
    new_ptr = (BKVertexList *) bk_malloc(list_size);
    bk_wmemcpy(new_ptr, this, list_size);
    return new_ptr;
}

void vtxList_copyColorsFrom(BKVertexList *this, BKVertexList *src) {
    Vtx *dst_start_ptr, *dst_end_ptr, *i_ptr, *src_ptr;

    src_ptr = src->vertices;
    dst_start_ptr = this->vertices;
    dst_end_ptr = dst_start_ptr + this->count;

    for (i_ptr = dst_start_ptr; i_ptr < dst_end_ptr; i_ptr++, src_ptr++) {
            i_ptr->v.cn[0] = src_ptr->v.cn[0];
            i_ptr->v.cn[1] = src_ptr->v.cn[1];
            i_ptr->v.cn[2] = src_ptr->v.cn[2];
            i_ptr->v.cn[3] = src_ptr->v.cn[3];
    }

    osWritebackDCache(dst_start_ptr, (dst_end_ptr - dst_start_ptr) * sizeof(Vtx));
}

void vtxList_tintColorsFrom(BKVertexList *this, s32 target_color[3], f32 intensity, BKVertexList *src) {
    Vtx *dst_start_ptr, *dst_end_ptr, *i_ptr, *src_ptr;
    s32 i;

    dst_start_ptr = this->vertices;
    dst_end_ptr = dst_start_ptr + this->count;

    for (i_ptr = dst_start_ptr, src_ptr = src->vertices; i_ptr < dst_end_ptr; i_ptr++, src_ptr++){
        for (i = 0; i < 3; i++) {
            i_ptr->v.cn[i] = src_ptr->v.cn[i] + (target_color[i] - src_ptr->v.cn[i]) * intensity;
        }
    }

    osWritebackDCache(dst_start_ptr, (dst_end_ptr - dst_start_ptr) * sizeof(Vtx));
}

/**
 * @note Seems Recolor vtx based on how "in view" a vtx is
 * 
 * @param dst vertexList to recolor
 * @param src vertexList to tak colors from
 * @param position vertexlist world position
 * @param rotation vertexlist world rotation
 * @param arg4 Step function definition 
 */
void vtxList_func_802ECBD4(BKVertexList *dst, BKVertexList *src, f32 position[3], f32 rotation[3], f32 arg4[4]) {
    f32 vp_position[3];
    f32 vp_look[3];
    Vtx *dst_vtx;
    Vtx *start_vtx;
    Vtx *end_vtx;
    Vtx *src_vtx;
    f32 sp4C[3];
    s32 i;
    f32 temp_f0;
    s32 pad40;

    viewport_getPosition_vec3f(vp_position);
    viewport_getLookVector(vp_look);
    mlMtxIdent();
    func_80252CC4(position, rotation, 1.0f, NULL);
    mlMtx_apply_vec3f(vp_position, vp_position);

    mlMtxIdent();
    func_80252CC4(NULL, rotation, 1.0f, NULL);
    mlMtx_apply_vec3f(vp_look, vp_look);

    start_vtx = (Vtx *)(dst + 1);
    end_vtx = start_vtx + dst->count;
    for(dst_vtx = start_vtx, src_vtx = (Vtx *)(src + 1); dst_vtx < end_vtx; dst_vtx++, src_vtx++){
            TUPLE_DIFF_COPY(sp4C, dst_vtx->v.ob, vp_position);
            temp_f0 = TUPLE_DOT_PRODUCT(vp_look, sp4C);
            temp_f0 = core2_C3A40_getIntensity(arg4, temp_f0);
            for(i = 0; i < 3; i++){
                dst_vtx->v.cn[i] = temp_f0*src_vtx->v.cn[i];
            }
    }
    osWritebackDCache(start_vtx, (end_vtx - start_vtx) * sizeof(Vtx));
}

void vtxList_randColors(BKVertexList *this) {
    Vtx *start_ptr;
    Vtx *end_ptr;
    Vtx *i_ptr;
    s32 i;

    start_ptr = this->vertices;
    end_ptr = start_ptr + this->count;
    for (i_ptr = start_ptr; i_ptr < end_ptr; i_ptr++) {
        for (i = 0; i < 3; i++) {
            i_ptr->v.cn[i] = 0;
        }
        i_ptr->v.cn[0] = randf() * 255.0f;
    }
    osWritebackDCache(start_ptr, (end_ptr - start_ptr) * sizeof(Vtx));
}

void vtxList_randWalkColor(BKVertexList *self) {
    Vtx *start_ptr;
    Vtx *end_ptr;
    Vtx *i_ptr;
    s32 i;
    s32 phi_s0;

    start_ptr = &self->vertices[0];
    end_ptr = start_ptr + self->count;
    for(i_ptr = start_ptr; i_ptr < end_ptr; i_ptr++){
        for(i = 0; i < 3; i++){
            phi_s0 = i_ptr->v.cn[i];
            phi_s0 += randf()*21.0f - 10.0f;
            phi_s0 = (phi_s0 > 0xff) ? 0xff : phi_s0;
            phi_s0 =  (phi_s0 < 0) ? 0 : phi_s0;
            i_ptr->v.cn[i] = phi_s0;
        }
    }
    osWritebackDCache(start_ptr, ((s32)(end_ptr - start_ptr)) * sizeof(Vtx));
}

void vtxList_recolor(BKVertexList *this, s32 color[3]) {
    Vtx *start_ptr, *end_ptr, *i_ptr;
    int i;

    start_ptr = this->vertices;
    end_ptr = start_ptr + this->count;

    for (i_ptr = start_ptr; i_ptr < end_ptr; i_ptr++) {
        for (i = 0; i < 3; i++) {
            i_ptr->v.cn[i] = color[i];
        }
    }

    osWritebackDCache(start_ptr, (end_ptr - start_ptr) * sizeof(Vtx));
}

void vtxList_func_802ED108(f32 arg0[3]){
    arg0[0] = D_803808C0.unk20[0];
    arg0[1] = D_803808C0.unk20[1];
    arg0[2] = D_803808C0.unk20[2];
}

s32 vtxList_func_802ED12C(void){
    return  D_803808C0.unk0;
}

void vtxList_func_802ED138(f32 arg0[3], f32 arg1[3], f32 arg2){
    D_803808C0.unk0 = 0;
    D_803808C0.unk10[0] = arg0[0];
    D_803808C0.unk10[1] = arg0[1];
    D_803808C0.unk10[2] = arg0[2];

    D_803808C0.unk4[0] = arg1[0];
    D_803808C0.unk4[1] = arg1[1];
    D_803808C0.unk4[2] = arg1[2];

    D_803808C0.unk1C = arg2;
}

void vtxList_func_802ED180(BKVertexList *self, f32 arg1[3], f32 arg2[3], f32 arg3, f32 arg4[3]) {
    Vtx *phi_s0;
    f32 sp88[3];
    f32 sp7C[3];
    f32 sp70[3];
    Vtx *phi_s1;
    Vtx *start;
    f32 sp5C[3];
    f32 temp_f20;
    f32 temp_f0;


    mlMtxIdent();
    func_80252CC4(arg1, arg2, arg3, arg4);
    mlMtx_apply_vec3f(sp88, D_803808C0.unk10);
    mlMtx_apply_vec3f(sp7C, D_803808C0.unk4);
    temp_f20 = D_803808C0.unk1C / arg3;
    temp_f20 = temp_f20*temp_f20;
    start = (Vtx*)(self + 1);
    phi_s1 = start + self->count;
    for(phi_s0 = start; phi_s0 < phi_s1; phi_s0++){
        sp70[0] = (f32) phi_s0->v.ob[0];
        sp70[1] = (f32) phi_s0->v.ob[1];
        sp70[2] = (f32) phi_s0->v.ob[2];

        sp5C[0] = sp70[0] - sp7C[0];
        sp5C[1] = sp70[1] - sp7C[1];
        sp5C[2] = sp70[2] - sp7C[2];

        temp_f0 = sp5C[0]*sp5C[0] + sp5C[1]*sp5C[1] + sp5C[2]*sp5C[2];
        if (!(temp_f20 <= temp_f0)) {
            D_803808C0.unk0 = 1;
            mlMtxIdent();
            func_80252C08(arg1, arg2, arg3, arg4);
            mlMtx_apply_vec3f(D_803808C0.unk20, sp70);
        }
    }
}

void vtxList_getNthCoord(BKVertexList *this, u32 index, f32 dst[3]) {
    Vtx *vtx;

    vtx = (s32)(this + 1) + (index * sizeof(Vtx));
    dst[0] = (f32) vtx->v.ob[0];
    dst[1] = (f32) vtx->v.ob[1];
    dst[2] = (f32) vtx->v.ob[2];
}

void vtxList_setNthCoord(BKVertexList *this, u32 index, f32 coords[3]) {
    Vtx *vtx;
    s32 i;

    vtx = ((Vtx *) (this + 1)) + index;
    for (i = 0; i < 3; i++) {
        vtx->v.ob[i] = (coords[i] >= 0.0) ? coords[i] + 0.5 :  coords[i] - 0.5;
    }
}
