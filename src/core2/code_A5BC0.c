...
s16 remove_or_return_noteIndex(Prop *other_prop, bool remove) {
    ...
    for (i = 0; i < noteTotal; i++) {
        if ((other_prop->position_x == currNotePositions[i][0]) && 
            (other_prop->position_y == currNotePositions[i][1]) && 
            (other_prop->position_z == currNotePositions[i][2])) {
            if (remove) {
...
bool check_if_note(Prop *other_prop){
    s32 temp_v0_5 = other_prop->spriteProp.spriteId + SPRITE_ASSET_OFFSET;
