...
bool cutscene_skipEndBeach2CutsceneCheck(void) {
    if (volatileFlag_get(VOLATILE_FLAG_64_DEMO_FINISHED_VIA_START_BTN)) {
        volatileFlag_set(VOLATILE_FLAG_64_DEMO_FINISHED_VIA_START_BTN, FALSE);
    }
...
bool cutscene_skipEndSprialMountainCutscenesCheck(void) {
    if ((controller_getStartButtonSafe(0) == 1) && !gctransition_8030BDC0()) {
        if (mapSpecificFlags_get(1) && !mapSpecificFlags_get(0xC)) {
            return TRUE;
...
#if !(defined(SKIPPABLE_CUTSCENES) && !defined(OPTIONS_MENU))
...
#ifdef SKIPPABLE_CUTSCENES
    if ((getGameMode() == GAME_MODE_A_SNS_PICTURE) && volatileFlag_get(VOLATILE_FLAG_64_DEMO_FINISHED_VIA_START_BTN)
 #ifdef OPTIONS_MENU
        && is_qol_feature_enabled(QOL_ID_SKIPPABLE_CUTSCENES)
 #endif
        ) {
